...
export default function ShowResults() {
    ...
    const fetchResults = async (tanggal = '') => {
        try {
            const response = await fetch(`/api/showData2${tanggal ? `?tanggal=${tanggal}` : ''}`);
            if (response.ok) {
    ...
    const handleDelete = async (id) => {
        if (confirm('Are you sure you want to delete this data?')) {
            try {
                const response = await fetch(`/api/deleteData2?id=${id}`, {
                    method: 'DELETE',
                });
                if (response.ok) {
    ...
    return (
        <>
            ...
            <div className="calculator-title">
                <h2 className="mt-5 mb-4 text-center">MTBF & MTTR Data LINE 2</h2>
            </div>
            ...
            <div id='results-table-container'>
                ...
                <div className="d-flex justify-content-start mt-3">
                    <Link href="/line2" legacyBehavior>
